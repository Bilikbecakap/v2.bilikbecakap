...
const form = useForm({
  nama_kategori: props.gambar.nama_kategori,
  deskripsi: props.gambar.deskripsi,
  is_active: props.gambar.is_active,
  urutan: props.gambar.urutan,
});

const submit = () => {
  form.put(route("data-master.gambar.update", props.gambar.id), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  <Head title="Edit Master Gambar" />

  <AdminLayout>
    <template #title>Edit Master Gambar</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Ubah informasi kategori gambar
          </p>
        </div>

        <div class="flex gap-3">
          <Link
            :href="route('data-master.gambar.index')"
