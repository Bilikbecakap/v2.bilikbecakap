...
const selectedChart = ref('trend'); // 'trend', 'status', 'quiz'
</script>
...
<template>
    ...
    <AdminLayout>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-6 md:mb-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <Link :href="route('kamus.index')" class="text-blue-600 dark:text-blue-400 hover:text-blue-700">
                ...
                <div class="flex items-center gap-3 text-xs">
                    ...
                    <span class="inline-flex items-center text-orange-600 dark:text-orange-400" v-if="stats.kamus.pending > 0">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <Link :href="route('quiz.index')" class="text-cyan-600 dark:text-cyan-400 hover:text-cyan-700">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </Link>
                </div>
                <h3 class="text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Quizz Tersedia</h3>
                <p class="text-3xl font-bold text-slate-900 dark:text-white mb-3">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <Link :href="route('artikel.index')" class="text-green-600 dark:text-green-400 hover:text-green-700">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </Link>
                ...
                <div class="flex items-center gap-3 text-xs">
                    ...
                    <span class="inline-flex items-center text-orange-600 dark:text-orange-400" v-if="stats.artikel.pending > 0">
                        <span class="w-2 h-2 bg-orange-500 rounded-full mr-1"></span>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <Link :href="route('modul-pembelajaran.index')" class="text-orange-600 dark:text-orange-400 hover:text-orange-700">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                        </svg>
                    </Link>
            ...
            <div class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                ...
                <div class="p-6">
                    <div v-if="pendingItems.kamus.length === 0 && pendingItems.artikel.length === 0" 
                        ...
                        <svg class="w-12 h-12 text-slate-400 dark:text-slate-500 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div v-else class="space-y-3">
                        ...
                        <div v-for="item in pendingItems.kamus" :key="`kamus-${item.id}`"
                            ...
                            <div class="flex-shrink-0 mr-3">
                                <div class="w-10 h-10 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center">
                                    <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="getTypeIcon('kamus')" />
                                    </svg>
                                </div>
                            </div>
                            <div class="flex-1 min-w-0">
                                <Link :href="item.url" class="text-sm font-medium text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400">
                                    {{ item.title }}
                                </Link>
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 ml-2">
                                Kamus
                            </span>
                        </div>
                        ...
                        <div v-for="item in pendingItems.artikel" :key="`artikel-${item.id}`"
                            ...
                            <div class="flex-shrink-0 mr-3">
                                <div class="w-10 h-10 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center">
                                    <svg class="w-5 h-5 text-green-600 dark:text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="getTypeIcon('artikel')" />
                                    </svg>
                                </div>
                            </div>
                            <div class="flex-1 min-w-0">
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 ml-2">
                                Artikel
                            </span>
                        </div>
                    </div>
        ...
        <div v-if="topPerformers.most_viewed_artikel.length > 0 || topPerformers.most_viewed_modul.length > 0" 
             class="mt-6 md:mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            <!-- Most Viewed Artikel -->
            <div v-if="topPerformers.most_viewed_artikel.length > 0" 
                ...
                <div class="space-y-3">
                    <Link v-for="(item, index) in topPerformers.most_viewed_artikel" :key="`top-artikel-${index}`"
                          :href="item.url"
                          class="flex items-center justify-between p-3 bg-white/50 dark:bg-slate-800/50 rounded-lg hover:bg-white dark:hover:bg-slate-800 transition-colors duration-150">
            ...
            <!-- Most Viewed Modul -->
            <div v-if="topPerformers.most_viewed_modul.length > 0" 
                ...
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white mb-4 flex items-center">
                    <svg class="w-5 h-5 text-orange-600 dark:text-orange-400 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                    Most Viewed Modul
                </h3>
                <div class="space-y-3">
                    <Link v-for="(item, index) in topPerformers.most_viewed_modul" :key="`top-modul-${index}`"
                          :href="item.url"
                          class="flex items-center justify-between p-3 bg-white/50 dark:bg-slate-800/50 rounded-lg hover:bg-white dark:hover:bg-slate-800 transition-colors duration-150">
                        <div class="flex items-center gap-3">
                            <span :class="[
                        ...
                        <span class="text-sm font-semibold text-orange-600 dark:text-orange-400 flex items-center">
                            <svg class="w-4 h-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            </svg>
