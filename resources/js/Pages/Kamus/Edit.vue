...
const props = defineProps({
    kamus: { type: Object, required: true },
    hasValidationPermission: { type: Boolean, default: false }
});

const form = useForm({
    kata: props.kamus.kata,
    pos: props.kamus.pos || '',
    definisi: props.kamus.definisi,
    catatan_validasi: props.kamus.catatan_validasi || '',
    status: props.kamus.status,
    audio: null,
    contoh: props.kamus.contoh
        ? props.kamus.contoh.map(c => ({ kalimat: c.contoh_kalimat, arti: c.arti_contoh_kalimat || '' }))
        : [],
});
...
// --- Contoh kalimat ---
const addContoh = () => { form.contoh.push({ kalimat: '', arti: '' }); };
const removeContoh = (index) => { form.contoh.splice(index, 1); };
...
const startRecording = async () => {
    try {
        ...
        mediaRecorder.value.onstop = () => {
            ...
            const ext = mimeType === 'audio/mp4' ? '.m4a' : '.wav';
            audioBlob.value = new Blob(audioChunks.value, { type: mimeType });
            audioUrl.value = URL.createObjectURL(audioBlob.value);
            const file = new window.File([audioBlob.value], `recording_${Date.now()}${ext}`, { type: mimeType });
            form.audio = file;
...
const clearRecording = () => {
    ...
    audioUrl.value = null; audioBlob.value = null; form.audio = null;
    audioFileName.value = ''; recordingTime.value = 0;
...
const handleFileUpload = (event) => {
    ...
    if (file.size > 10240 * 1024) { alert('Ukuran file terlalu besar. Maksimal 10MB.'); event.target.value = ''; return; }
...
const playAudio = () => {
    const src = audioUrl.value || (form.audio instanceof File ? URL.createObjectURL(form.audio) : null) || existingAudioUrl.value;
    if (!src) return;
    const audio = new Audio(src);
    audio.play().catch(() => {});
    if (!audioUrl.value && !existingAudioUrl.value) audio.onended = () => URL.revokeObjectURL(src);
};

const playExistingAudio = () => {
    if (!existingAudioUrl.value) return;
    new Audio(existingAudioUrl.value).play().catch(() => {});
};

const removeAudio = () => {
    form.audio = null; audioFileName.value = '';
    if (audioUrl.value) { URL.revokeObjectURL(audioUrl.value); audioUrl.value = null; audioBlob.value = null; }
...
const submit = () => {
    const submitData = {
        kata: form.kata,
        pos: form.pos || '',
        definisi: form.definisi,
        catatan_validasi: form.catatan_validasi || '',
        status: form.status,
        audio: form.audio,
        contoh: form.contoh,
        ...
        <div class="mb-6 md:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Edit informasi kata dalam Kamus Melayu Belitung</p>
                </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <!-- Status (hanya untuk user dengan permission validasi) -->
                    <div v-if="hasValidationPermission">
                        <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Status Kamus</label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label v-for="opt in statusOptions" :key="opt.value" :class="[
                                ...
                                <div :class="['w-4 h-4 rounded-full border-2 flex-shrink-0 mt-0.5 mr-3 flex items-center justify-center',
                                    form.status === opt.value
                                        ? opt.value === 1 ? 'border-green-500' : opt.value === 2 ? 'border-red-500' : opt.value === 3 ? 'border-yellow-500' : 'border-blue-500'
                                        : 'border-slate-300 dark:border-slate-600']">
                                    <div v-if="form.status === opt.value" :class="['w-2 h-2 rounded-full',
                                        opt.value === 1 ? 'bg-green-500' : opt.value === 2 ? 'bg-red-500' : opt.value === 3 ? 'bg-yellow-500' : 'bg-blue-500']"></div>
                                </div>
                                <div>
                                    <p :class="['text-sm font-semibold',
                                        form.status === opt.value
                                            ? opt.value === 1 ? 'text-green-800 dark:text-green-300' : opt.value === 2 ? 'text-red-800 dark:text-red-300' : opt.value === 3 ? 'text-yellow-800 dark:text-yellow-300' : 'text-blue-800 dark:text-blue-300'
                                            : 'text-slate-700 dark:text-slate-300']">{{ opt.label }}</p>
                ...
                <div class="p-6">
                    ...
                    <div v-for="(item, index) in form.contoh" :key="index"
                        ...
                        <div class="flex items-center justify-between mb-3">
                            ...
                            <button type="button" @click="removeContoh(index)" class="p-1.5 text-red-500 hover:bg-red-100 dark:hover:bg-red-900/30 rounded-lg transition-colors duration-200">
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <textarea v-model="item.kalimat" rows="2" placeholder="Contoh kalimat dalam Bahasa Melayu Belitung"
                            ...
                            <div>
                                ...
                                <textarea v-model="item.arti" rows="2" placeholder="Terjemahan kalimat dalam Bahasa Indonesia"
                                    class="block w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-colors duration-200 resize-none"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                    ...
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Edit audio pengucapan untuk kata Melayu Belitung</p>
                </div>
                <div class="p-6">
                    <!-- Existing Audio -->
                    <div v-if="hasExistingAudio && existingAudioUrl" class="mb-6 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-blue-800 dark:text-blue-300">Audio Saat Ini</p>
                                    <p class="text-xs text-blue-600 dark:text-blue-400">Audio pengucapan yang ada</p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <button type="button" @click="playExistingAudio" class="p-2 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-800 rounded-lg transition-colors duration-200" title="Putar Audio">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </button>
                                <button type="button" @click="removeExistingAudio" class="p-2 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-800 rounded-lg transition-colors duration-200" title="Hapus Audio">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div v-if="uploadMethod === 'upload'">
                        <div class="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg p-6 text-center">
                            ...
                            <label for="audio-upload" class="cursor-pointer">
                                <span class="text-blue-600 dark:text-blue-400 hover:text-blue-800 font-medium">Pilih file audio</span>
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">MP3, WAV, OGG, M4A (Maksimal 10MB)</p>
                        </div>
                    </div>
                    ...
                    <!-- New Audio Preview -->
                    <div v-if="form.audio && form.audio !== '' && form.audio?.constructor?.name === 'File'" class="mt-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="text-sm font-medium text-green-800 dark:text-green-300">{{ audioFileName || 'Audio rekaman baru' }}</p>
                                    <p class="text-xs text-green-600 dark:text-green-400">Audio baru siap untuk disimpan</p>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <button type="button" @click="playAudio" class="p-2 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-800 rounded-lg transition-colors duration-200">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </button>
                                <button type="button" @click="uploadMethod === 'upload' ? removeAudio() : clearRecording()" class="p-2 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-800 rounded-lg transition-colors duration-200">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
