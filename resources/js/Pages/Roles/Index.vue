    ...
    <AdminLayout>
        ...
        <div class="mb-6 md:mb-8">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">All Roles</h2>
                    <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Manage system roles and their permissions</p>
                </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 md:p-6">
                <div class="flex items-center">
                    ...
                    <div>
                        <p class="text-sm font-medium text-slate-600 dark:text-slate-400">With Permissions</p>
                        <p class="text-2xl font-bold text-slate-900 dark:text-white">{{roles.filter(role => role.permissions &&
                            role.permissions.length > 0).length}}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4 md:p-6">
                <div class="flex items-center">
                    <div
                        ...
                        <svg class="w-6 h-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 9l3 3-3 3m13 0h-6m-8-4h6m4 0V9a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h14a2 2 0 002-2z" />
                        </svg>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-slate-600 dark:text-slate-400">Total Permissions</p>
                        <p class="text-2xl font-bold text-slate-900 dark:text-white">{{roles.reduce((total, role) => total +
                            (role.permissions
                                ? role.permissions.length : 0), 0)}}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
                <h3 class="text-lg font-semibold text-slate-800 dark:text-white">Roles List</h3>
            </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-sm text-slate-500 dark:text-slate-400">Role ID: {{ role.id }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
