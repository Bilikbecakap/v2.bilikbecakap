...
<template>
  ...
  <AdminLayout>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Kelola artikel dan konten blog sistem
          </p>
        </div>
    ...
    >
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-start space-x-4">
                  <img
                    :src="getThumbnailUrl(item.gambar_thumbnail)"
                    :alt="getMainTitle(item)"
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1">
                      ...
                      <span
                        ...
                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200"
                      >
                    ...
                    <div class="flex flex-wrap gap-1">
                      <span
                        v-if="item.judul_indonesia"
                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300"
                      >
                        ID
                      </span>
                      <span
                        v-if="item.judul_melayu"
                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-300"
                      >
                        MS
                      </span>
                      <span
                        v-if="item.judul_english"
                        class="inline-flex items-center px-1.5 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300"
                      >
                        EN
                      </span>
                    </div>
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <div class="flex items-center justify-end gap-2">
                  ...
                  <!-- Approval Actions -->
                  <div
                    v-if="item.can_approve && item.status === 'pending'"
                    class="flex gap-1"
                  >
                    <button
                      @click="approveArtikel(item)"
                      class="inline-flex items-center px-2 py-1 text-xs font-medium text-green-600 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 border border-green-200 dark:border-green-700 rounded hover:bg-green-50 dark:hover:bg-green-900/20 transition-colors duration-150"
                    >
                      <svg
                        class="w-3 h-3 mr-1"
                        fill="none"
                    ...
                    <button
                      @click="rejectArtikel(item)"
                      class="inline-flex items-center px-2 py-1 text-xs font-medium text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 border border-red-200 dark:border-red-700 rounded hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors duration-150"
                    >
                      <svg
                        class="w-3 h-3 mr-1"
                        fill="none"
      ...
      <!-- Pagination -->
      <div
        v-if="artikel.data && artikel.data.length > 0"
      ...
      >
        <div class="flex items-center justify-between">
          <div class="text-sm text-slate-600 dark:text-slate-400">
            Menampilkan {{ artikel.from }} sampai {{ artikel.to }} dari
            {{ artikel.total }} data
          </div>
          ...
          <div class="flex items-center space-x-2">
            <!-- Previous Button -->
            <Link
              v-if="artikel.prev_page_url"
              :href="artikel.prev_page_url"
            ...
            <div class="hidden sm:flex items-center space-x-1">
              <template v-for="(link, index) in artikel.links" :key="index">
                <Link
            ...
            <!-- Next Button -->
            <Link
              v-if="artikel.next_page_url"
              :href="artikel.next_page_url"
              class="inline-flex items-center px-3 py-2 text-sm font-medium text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors duration-150"
            >
              Next
              <svg
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-slate-500 dark:text-slate-400">
                  ...
                  <strong>{{ getMainTitle(selectedArtikel) }}</strong
