...
const submit = () => {
  form.post(route("data-master.modul.store"), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  <Head title="Tambah Master Modul" />

  <AdminLayout>
    <template #title>Tambah Master Modul</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">
            Tambah Kategori Modul
          </h2>
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Buat kategori modul pembelajaran baru untuk sistem
          </p>
        </div>
