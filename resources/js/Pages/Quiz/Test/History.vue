...
const confirmDelete = () => {
  if (selectedAttempt.value) {
    router.delete(
      route("quiz.attempts.destroy", [props.quiz.id, selectedAttempt.value.id]),
      {
        onSuccess: () => {
        ...
        >
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-slate-500 dark:text-slate-400">
                  Apakah Anda yakin ingin menghapus data pengerjaan dari
                  <strong>{{ selectedAttempt?.participant_name }}</strong>?
                  Semua jawaban dan hasil akan ikut terhapus. Tindakan ini tidak dapat dibatalkan.
                </p>
              </div>
            </div>
          </div>
