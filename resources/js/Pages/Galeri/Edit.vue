...
const form = useForm({
  gambar: null,
  master_gambar_id: props.galeri.master_gambar_id,
  keterangan: props.galeri.keterangan,
  is_active: props.galeri.is_active,
  remove_gambar: false,
  _method: 'PUT', // Tambahkan ini
});

const imagePreview = ref(props.galeri.gambar_url);
...
const removeNewImage = () => {
  form.gambar = null;
  form.remove_gambar = false;
  imagePreview.value = props.galeri.gambar_url;
...
const submit = () => {
  form.post(route("galeri.update", props.galeri.id), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  ...
  <AdminLayout>
    <template #title>Edit Gambar Galeri</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Perbarui informasi gambar galeri
          </p>
        </div>
    ...
    >
      ...
      <form @submit.prevent="submit" class="p-6 space-y-6">
        <!-- Current Image -->
        <div v-if="showExistingImage && galeri.gambar_url">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
            Ganti Gambar <span class="text-slate-500 text-xs">(Opsional - kosongkan jika tidak ingin mengganti)</span>
          </label>

          <!-- Preview New Image -->
          <div v-if="imagePreview && !showExistingImage" class="mb-4 relative inline-block">
            <img
              :src="imagePreview"
              alt="Preview"
              class="w-full max-w-md h-64 object-cover rounded-lg border-2 border-blue-200 dark:border-blue-600"
            />
            <button
              type="button"
        ...
        <!-- Hapus Gambar -->
        <div v-if="galeri.gambar_url && !form.gambar">
          <button
            type="button"
            @click="removeImage"
            class="inline-flex items-center px-4 py-2 text-sm font-medium text-red-600 dark:text-red-400 border border-red-200 dark:border-red-700 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
          >
