...
const submit = () => {
  form.post(route("data-master.gambar.store"), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  <Head title="Tambah Master Gambar" />

  <AdminLayout>
    <template #title>Tambah Master Gambar</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">
            Tambah Kategori Gambar
          </h2>
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Buat kategori gambar baru untuk sistem
          </p>
        </div>
