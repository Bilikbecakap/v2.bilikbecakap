...
const form = useForm({
  audio: null,
  keterangan: props.musicQuiz.keterangan,
  is_active: props.musicQuiz.is_active,
  _method: 'PUT', // Tambahkan ini
});

const audioFileName = ref("");
const audioPreview = ref(null);
const existingAudio = ref(props.musicQuiz.audio_url);
const showExistingAudio = ref(true);
...
const submit = () => {
  form.post(route("data-master.quiz-music.update", props.musicQuiz.id), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  ...
  <Head title="Edit Media Music Quiz" />

  <AdminLayout>
    <template #title>Edit Media Music Quiz</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 class="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">
            Edit Media Audio Quiz
          </h2>
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Ubah informasi media audio quiz
          </p>
        </div>
    ...
    >
      ...
      <form @submit.prevent="submit" class="p-6 space-y-6">
        ...
        <div>
          ...
          >
            Ganti File Audio <span class="text-slate-500 text-xs">(Opsional - kosongkan jika tidak ingin mengganti)</span>
          </label>
          ...
          <!-- Preview New Audio -->
          <div v-if="audioFileName" class="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border-2 border-blue-200 dark:border-blue-800">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3 flex-1">
                <div class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z" />
                  </svg>
                </div>
