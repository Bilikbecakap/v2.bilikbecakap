...
const form = useForm({
  nama_kategori: props.modul.nama_kategori,
  deskripsi: props.modul.deskripsi,
  is_active: props.modul.is_active,
  urutan: props.modul.urutan,
});

const submit = () => {
  form.put(route("data-master.modul.update", props.modul.id), {
    onSuccess: () => {
      // Handle success
    },
  });
...
<template>
  <Head title="Edit Master Modul" />

  <AdminLayout>
    <template #title>Edit Master Modul</template>
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">
            Ubah informasi kategori modul pembelajaran
          </p>
        </div>

        <div class="flex gap-3">
          <Link
            :href="route('data-master.modul.index')"
